...
TEST(Node, SmokeTest) {
    Node *n = new Node("0.0.0.0", std::to_string(8080));
    delete n;
}
...
TEST(Node, InvalidAddress) {
    ...
    {
        try {
            Node n(addr.c_str(), std::to_string(8080));
        }
...
TEST(Node, SocketLeakage)
{
...

    {
        ...
        for (int i=0; i<100; ++i)
        {
            nodes.emplace_back("255.255.255.255", std::to_string(8080));
        }
        PAUSE(1000);
        std::stringstream ss;
        ss << "Using " << get_number_of_sockets() << " sockets";
...
TEST(Node, SocketAllocError)
{
    ...
    try {
        ...
        {
            nodes.emplace_back(Node("255.255.255.255", std::to_string(8080)));
            i++;
            if (i % 1000 == 0)
            {
